...
namespace Microsoft {
    namespace KinectBridge {
        ...
        class KinectHelper
        {
			...
			static const NUI_IMAGE_RESOLUTION DEPTH_DEFAULT_RESOLUTION = NUI_IMAGE_RESOLUTION_640x480;
        ...
        public:
			...
			void  GetCoordinateMapper(INuiCoordinateMapper** mapper);
			NUI_LOCKED_RECT* GetColorLockRect();
			NUI_LOCKED_RECT* GetDepthLockRect();
        ...
        private:
			...
			NUI_LOCKED_RECT *m_colorLockRect;
			NUI_LOCKED_RECT *m_depthLockRect;
		...
		KinectHelper<Image>::KinectHelper() :
			...
			m_pNuiSensor(NULL),
			m_colorLockRect(NULL),
			m_depthLockRect(NULL),
            m_pColorBuffer(NULL),
            m_colorBufferSize(0),
        ...
        HRESULT KinectHelper<Image>::Initialize(INuiSensor* pNuiSensor)
        {
            ...
            if (m_isUsingColor) 
            {
				...
				m_colorLockRect = new NUI_LOCKED_RECT();
            ...
            if (m_isUsingDepth) 
            {
				...
				m_depthLockRect = new NUI_LOCKED_RECT();
        ...
        void KinectHelper<Image>::UnInitialize()
        {
            ...
            if (m_pNuiSensor)
            {
				...
				delete m_colorLockRect;
				m_colorLockRect = NULL;

				delete m_depthLockRect;
				m_depthLockRect = NULL;
            }
        ...
        HRESULT KinectHelper<Image>::UpdateColorFrame(DWORD waitMillis /* = 0 */)
        {
			...
			pTexture->LockRect(0, m_colorLockRect, NULL, 0);

            // Check if image is valid
			if (m_colorLockRect->Pitch != 0)
            {
                // Copy image information into buffer so it doesn't get overwritten later
				BYTE* pBuffer = m_colorLockRect->pBits;
				INT size = m_colorLockRect->size;
				INT pitch = m_colorLockRect->Pitch;
        ...
        HRESULT KinectHelper<Image>::UpdateDepthFrame(DWORD waitMillis /* = 0 */)
        {
            ...
            pTexture->LockRect(0, m_depthLockRect, NULL, 0);

            // Check if image is valid
			if (m_depthLockRect->Pitch != 0)
            {
                // Copy image information into buffer
				BYTE* pBuffer = m_depthLockRect->pBits;
				INT size = m_depthLockRect->size;
				INT pitch = m_depthLockRect->Pitch;
		...
		template <typename Image>
		void KinectHelper<Image>::GetCoordinateMapper(INuiCoordinateMapper** mapper)
		{
			if (m_pNuiSensor)
			{
				m_pNuiSensor->NuiGetCoordinateMapper(mapper);
			}
		}

		template <typename Image>
		NUI_LOCKED_RECT* KinectHelper<Image>::GetColorLockRect()
		{ 
			return m_colorLockRect; 
		}

		template <typename Image>
		NUI_LOCKED_RECT* KinectHelper<Image>::GetDepthLockRect()
		{
			return m_depthLockRect;
		}
}
}
