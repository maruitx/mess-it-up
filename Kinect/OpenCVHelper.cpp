...
const cv::Scalar OpenCVHelper::SKELETON_COLORS[NUI_SKELETON_COUNT] =
{
	cv::Scalar(0, 255, 0),      // Green
    cv::Scalar(255, 0, 0),      // Blue
    cv::Scalar(64, 255, 255),   // Yellow
    cv::Scalar(255, 255, 64),   // Light blue
    cv::Scalar(255, 64, 255),   // Purple
    cv::Scalar(128, 128, 255)   // Pink
};
...
HRESULT OpenCVHelper::DrawSkeletonsInColorImage(Mat* pImg, const QVector<Vector4> &skeletonData, NUI_IMAGE_RESOLUTION colorResolution, NUI_IMAGE_RESOLUTION depthResolution)
{
	DrawSkeleton(pImg, skeletonData, SKELETON_COLORS[0], colorResolution, depthResolution);
...
HRESULT OpenCVHelper::DrawSkeletonsInDepthImage(Mat* pImg, const QVector<Vector4> &skeletonData, NUI_IMAGE_RESOLUTION depthResolution)
{
	DrawSkeleton(pImg, skeletonData, SKELETON_COLORS[0], NUI_IMAGE_RESOLUTION_INVALID, depthResolution);
                                    ...
                                    NUI_IMAGE_RESOLUTION depthResolution)
{
    ...
    for (int i=0; i < NUI_SKELETON_COUNT; ++i)
    {
        ...
        else if (trackingState == NUI_SKELETON_POSITION_INFERRED) 
        {
            ...
            circle(*pImg, cv::Point(x, y), 7, SKELETON_COLORS[i], -1);
        }
    }
...
/// <param name="depthRes">resolution of depth image stream</param>
/// <returns>S_OK if successful, an error code otherwise</returns>
void OpenCVHelper::DrawSkeleton(Mat* pImg, NUI_SKELETON_DATA* pSkel, cv::Scalar color, NUI_IMAGE_RESOLUTION colorResolution,
                                NUI_IMAGE_RESOLUTION depthResolution)
{
    // Convert joint positions into the coordinates for this resolution and view
    cv::Point jointPositions[NUI_SKELETON_POSITION_COUNT];

    for (int j = 0; j < NUI_SKELETON_POSITION_COUNT; ++j)
    {
        LONG x, y;
        GetCoordinatesForSkeletonPoint(pSkel->SkeletonPositions[j], &x, &y, colorResolution, depthResolution);
        jointPositions[j] = cv::Point(x, y);
    }
    ...
    for (int j = 0; j < NUI_SKELETON_POSITION_COUNT; ++j)
    {
        ...
        {
            circle(*pImg, jointPositions[j], 5, color, -1);
            circle(*pImg, jointPositions[j], 6, cv::Scalar(0, 0, 0), 1);
        } 
        ...
        {
            circle(*pImg, jointPositions[j], 4, cv::Scalar(255,255,255), 2);
        }
    }
}

void OpenCVHelper::DrawSkeleton(Mat* pImg, const QVector<Vector4> &skeletonData, cv::Scalar color, NUI_IMAGE_RESOLUTION colorResolution, NUI_IMAGE_RESOLUTION depthResolution)
{
	// Convert joint positions into the coordinates for this resolution and view
	cv::Point jointPositions[NUI_SKELETON_POSITION_COUNT];
	...
	{
		LONG x, y;
		GetCoordinatesForSkeletonPoint(skeletonData[j], &x, &y, colorResolution, depthResolution);
		jointPositions[j] = cv::Point(x, y);
	}

	// Draw torso
	DrawBone(pImg, skeletonData, NUI_SKELETON_POSITION_HEAD, NUI_SKELETON_POSITION_SHOULDER_CENTER, jointPositions, color);
	DrawBone(pImg, skeletonData, NUI_SKELETON_POSITION_SHOULDER_CENTER, NUI_SKELETON_POSITION_SHOULDER_LEFT, jointPositions, color);
	DrawBone(pImg, skeletonData, NUI_SKELETON_POSITION_SHOULDER_CENTER, NUI_SKELETON_POSITION_SHOULDER_RIGHT, jointPositions, color);
	DrawBone(pImg, skeletonData, NUI_SKELETON_POSITION_SHOULDER_CENTER, NUI_SKELETON_POSITION_SPINE, jointPositions, color);
	DrawBone(pImg, skeletonData, NUI_SKELETON_POSITION_SPINE, NUI_SKELETON_POSITION_HIP_CENTER, jointPositions, color);
	DrawBone(pImg, skeletonData, NUI_SKELETON_POSITION_HIP_CENTER, NUI_SKELETON_POSITION_HIP_LEFT, jointPositions, color);
	DrawBone(pImg, skeletonData, NUI_SKELETON_POSITION_HIP_CENTER, NUI_SKELETON_POSITION_HIP_RIGHT, jointPositions, color);

	// Draw left arm
	DrawBone(pImg, skeletonData, NUI_SKELETON_POSITION_SHOULDER_LEFT, NUI_SKELETON_POSITION_ELBOW_LEFT, jointPositions, color);
	DrawBone(pImg, skeletonData, NUI_SKELETON_POSITION_ELBOW_LEFT, NUI_SKELETON_POSITION_WRIST_LEFT, jointPositions, color);
	DrawBone(pImg, skeletonData, NUI_SKELETON_POSITION_WRIST_LEFT, NUI_SKELETON_POSITION_HAND_LEFT, jointPositions, color);

	// Draw right arm
	DrawBone(pImg, skeletonData, NUI_SKELETON_POSITION_SHOULDER_RIGHT, NUI_SKELETON_POSITION_ELBOW_RIGHT, jointPositions, color);
	DrawBone(pImg, skeletonData, NUI_SKELETON_POSITION_ELBOW_RIGHT, NUI_SKELETON_POSITION_WRIST_RIGHT, jointPositions, color);
	DrawBone(pImg, skeletonData, NUI_SKELETON_POSITION_WRIST_RIGHT, NUI_SKELETON_POSITION_HAND_RIGHT, jointPositions, color);

	// Draw left leg
	DrawBone(pImg, skeletonData, NUI_SKELETON_POSITION_HIP_LEFT, NUI_SKELETON_POSITION_KNEE_LEFT, jointPositions, color);
	DrawBone(pImg, skeletonData, NUI_SKELETON_POSITION_KNEE_LEFT, NUI_SKELETON_POSITION_ANKLE_LEFT, jointPositions, color);
	DrawBone(pImg, skeletonData, NUI_SKELETON_POSITION_ANKLE_LEFT, NUI_SKELETON_POSITION_FOOT_LEFT, jointPositions, color);

	// Draw right leg
	DrawBone(pImg, skeletonData, NUI_SKELETON_POSITION_HIP_RIGHT, NUI_SKELETON_POSITION_KNEE_RIGHT, jointPositions, color);
	DrawBone(pImg, skeletonData, NUI_SKELETON_POSITION_KNEE_RIGHT, NUI_SKELETON_POSITION_ANKLE_RIGHT, jointPositions, color);
	DrawBone(pImg, skeletonData, NUI_SKELETON_POSITION_ANKLE_RIGHT, NUI_SKELETON_POSITION_FOOT_RIGHT, jointPositions, color);

	// Draw joints on top of bones
	for (int j = 0; j < NUI_SKELETON_POSITION_COUNT; ++j)
	{
		// Draw a colored circle with a black border for tracked joints
		if (skeletonData[j].w == NUI_SKELETON_POSITION_TRACKED)
		{
			circle(*pImg, jointPositions[j], 5, color, -1);
			circle(*pImg, jointPositions[j], 6, cv::Scalar(0, 0, 0), 1);
		}
		// Draw a white, unfilled circle for inferred joints
		else if (skeletonData[j].w == NUI_SKELETON_POSITION_INFERRED)
		{
			circle(*pImg, jointPositions[j], 4, cv::Scalar(255, 255, 255), 2);
		}
	}
...
void OpenCVHelper::DrawBone(Mat* pImg, NUI_SKELETON_DATA* pSkel, NUI_SKELETON_POSITION_INDEX joint0, 
                            NUI_SKELETON_POSITION_INDEX joint1, cv::Point jointPositions[NUI_SKELETON_POSITION_COUNT], cv::Scalar color)
{
    ...
    {
        line(*pImg, jointPositions[joint0], jointPositions[joint1], cv::Scalar(255,255,255), 1);
    }
}

void OpenCVHelper::DrawBone(Mat* pImg, const QVector<Vector4> &skeletonData, NUI_SKELETON_POSITION_INDEX joint0, NUI_SKELETON_POSITION_INDEX joint1, cv::Point jointPositions[NUI_SKELETON_POSITION_COUNT], cv::Scalar color)
{
	NUI_SKELETON_POSITION_TRACKING_STATE joint0state = NUI_SKELETON_POSITION_TRACKING_STATE((int)skeletonData[joint0].w);
	NUI_SKELETON_POSITION_TRACKING_STATE joint1state = NUI_SKELETON_POSITION_TRACKING_STATE((int)skeletonData[joint1].w);
