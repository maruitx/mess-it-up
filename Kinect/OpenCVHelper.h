...
class OpenCVHelper
{
    ...
    static const cv::Scalar SKELETON_COLORS[NUI_SKELETON_COUNT];
...
public:
	...
	HRESULT DrawSkeletonsInColorImage(Mat* pImg, const QVector<Vector4> &skeletonData,
		NUI_IMAGE_RESOLUTION colorResolution, NUI_IMAGE_RESOLUTION depthResolution);

	HRESULT DrawSkeletonsInDepthImage(Mat* pImg, const QVector<Vector4> &skeletonData,
		NUI_IMAGE_RESOLUTION depthResolution);
...
private:
    ...
    /// <param name="depthRes">resolution of depth image stream</param>
    /// <returns>S_OK if successful, an error code otherwise</returns>
    void DrawSkeleton(Mat* pImg, NUI_SKELETON_DATA* pSkel, cv::Scalar color, NUI_IMAGE_RESOLUTION colorResolution, 
        NUI_IMAGE_RESOLUTION depthResolution);

	void DrawSkeleton(Mat* pImg, const QVector<Vector4> &skeletonData, cv::Scalar color, NUI_IMAGE_RESOLUTION colorResolution,
		NUI_IMAGE_RESOLUTION depthResolution);
	...
	void DrawBone(Mat* pImg, const QVector<Vector4> &skeletonData, NUI_SKELETON_POSITION_INDEX joint0,
		NUI_SKELETON_POSITION_INDEX joint1, cv::Point jointPositions[NUI_SKELETON_POSITION_COUNT], cv::Scalar color);
