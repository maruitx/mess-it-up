...
int COBB::GetClosestAxis(const MathLib::Vector3 &v) const {
	...
	int iMAId(-1);
	for (int i=0; i<3; i++) {
		double dCosA = std::abs(axis[i].dot(vec));
		if (dCosA > dMaxCosA) {
			dMaxCosA = dCosA;
			iMAId = i;
		}
...
void COBB::CalcAxisMatchMap(const COBB &bb, std::vector<int> &m) const 
{
	...
	for (int i=0; i<3; i++) {
		MaxCosA[i].first = 0.0;
		for (int j=0; j<3; j++) {
			double dCosA = std::abs(axis[i].dot(bb.axis[j]));
...
void COBB::CalcAxisMatchBB(const COBB &bbr, COBB &bbo, std::vector<int> &m) const
{
	...
	for (int i=0; i<3; i++) {
		MaxCosA[i].first = 0.0;
		for (int j=0; j<3; j++) {
			double dCosA = std::abs(axis[i].dot(bbr.axis[j]));
			if (dCosA > MaxCosA[i].first) {
				MaxCosA[i].first = dCosA;
				MaxCosA[i].second.first = i;
				MaxCosA[i].second.second = j;
...
//	double dd = CalcHausdorffDist(bb)/dDMax;//cent.distance(bb.cent)/dDMax;
	...
	double dsl = Acos(Abs(la1.dot(la2)))/90.0 * (1.0-exp(-(dcl*0.5+dl1*0.25+dl2*0.25)*s));
	double dsp = Acos(Abs(sa1.dot(sa2)))/90.0 * (1.0-exp(-(dcp*0.5+dp1*0.25+dp2*0.25)*s));
...
int COBB::operator==(const COBB& other) {
	double d[3];
	d[0] = Abs(axis[0].dot(other.axis[0]));
	d[1] = Abs(axis[0].dot(other.axis[1]));
	d[2] = Abs(axis[0].dot(other.axis[2]));
	...
	for (int i=0; i<3; i++) {
		...
		d[di] = Abs(axis[1].dot(other.axis[i]));
		mi[di++] = i;
	}
	dm = __max(d[0],d[1]);
	if (dm < 0.98) {
	...
	mi3 = 3-mi1-mi2;
	dm = Abs(axis[2].dot(other.axis[mi3]));
	if (dm < 0.98) {
		return 0;
	}

	if ( !IsEqual(size[0], other.size[mi1], (size[0]+other.size[mi1])*0.05)
		|| !IsEqual(size[1], other.size[mi2], (size[1]+other.size[mi2])*0.05)
		|| !IsEqual(size[2], other.size[mi3], (size[2]+other.size[mi3])*0.05)) {
			return 0;
	}
	if ( !IsEqual(cent[0], other.cent[0], (size[0]+other.size[mi1])*0.05)
		|| !IsEqual(cent[1], other.cent[1], (size[0]+other.size[mi1])*0.05)
		|| !IsEqual(cent[2], other.cent[2], (size[0]+other.size[mi1])*0.05)) {
			return 0;
	}
...
void COBB::DrawBox(bool b3DEdge, bool bFace, bool bGraph, bool bShowStat, bool bHighL) const
{
	...
	if (bFace) {
		...
		for (int i=0; i<boxNumFace; i++) {
			...
			glVertex3dv(vp[boxFace[i][0]].v);
			glVertex3dv(vp[boxFace[i][1]].v);
			glVertex3dv(vp[boxFace[i][2]].v);
		}
		glEnd();
		glPopAttrib();
	}
...
double COBB::ConnStrength_Proj(const COBB &obb, const MathLib::Vector3 &dir) const
{
	...
	double dli = VI[boxFace[iFI][0]].distance(VI[boxFace[iFI][2]]);
	double dlj = VJ[boxFace[iFJ][0]].distance(VJ[boxFace[iFJ][2]]);
...
double COBB::TopHeightDiff(const COBB &obb, const MathLib::Vector3 &upright) const
{
	...
	for (unsigned int i=0; i<vp.size(); i++) {
		double t = upright.dot(vp[i]-ML_O);
		if (t > tmax1) {
			tmax1 = t;
		}
	}
	...
	for (unsigned int i=0; i<obb.vp.size(); i++) {
		double t = upright.dot(obb.vp[i]-ML_O);
		if (t > tmax2) {
			tmax2 = t;
		}
	}
	return Abs(tmax1-tmax2);
}
...
bool COBB::IsOnTop(const COBB &obb, const MathLib::Vector3 &upright, double &pd) const
{
...
// 				}
// 			}
			if (ContactTriTri(VI[boxFace[i][0]],VI[boxFace[i][1]],VI[boxFace[i][2]],FNI,VJ[boxFace[j][0]],VJ[boxFace[j][1]],VJ[boxFace[j][2]],FNJ,ta,td,true)) {
				dir = FNI;
				return true;
			}
		}
...
bool COBB::IsSupport(const COBB &obb, double ta, double td, const MathLib::Vector3 &upright) const
{
	...
	for (int i=0; i<boxNumFace; i++) {
		...
		for (int j=0; j<boxNumFace; j++) {
			FNJ.set(AJ[boxFaceNormalOrientAlongAxis[j][0]][0]*boxFaceNormalOrientAlongAxis[j][1],AJ[boxFaceNormalOrientAlongAxis[j][0]][1]*boxFaceNormalOrientAlongAxis[j][1],AJ[boxFaceNormalOrientAlongAxis[j][0]][2]*boxFaceNormalOrientAlongAxis[j][1]);
// 			if (IsTwoSide(obb,VI[psBoxF[i][0]],upright)) {
// 				if (ContactTriTri(VI[psBoxF[i][0]],VI[psBoxF[i][1]],VI[psBoxF[i][2]],FNI,VJ[psBoxF[j][0]],VJ[psBoxF[j][1]],VJ[psBoxF[j][2]],FNJ,td)) {
// 					return true;
// 				}
// 			}
			...
			if (ContactTriTri(VI[boxFace[j][0]], VI[boxFace[j][1]], VI[boxFace[j][2]], FNJ, VJ[boxFace[i][0]], VJ[boxFace[i][1]], VJ[boxFace[i][2]], FNI, ta, td, true)) {
				return true;
			}
		}
	}
...
bool COBB::PickByRay_Ortho(const MathLib::Vector3 &sp, const MathLib::Vector3 &dir, double &dPD) const
{
	...
	for (int i=0; i<boxNumFace; i++) {
		...
		if (Acos(fn.dot(dir)) < 179.0) {
			continue;
		}
		const MathLib::Vector3 &v1 = vp[boxFace[i][0]];
		const MathLib::Vector3 &v2 = vp[boxFace[i][1]];
		const MathLib::Vector3 &v3 = vp[boxFace[i][2]];
...
bool COBB::PickByRay(const MathLib::Vector3 &sp, const MathLib::Vector3 &dir, double &dPD) const
{
	MathLib::Vector3	e1, e2, p, s, q;
	double		t(0), u(0), v(0), w(0), tmp(0);
	int		pi = -1;
...
bool COBB::IsInteract(const std::vector<MathLib::Vector3> &pts, double dist, std::vector<int> &states)
{
	int num = pts.size();
	states.resize(num, 0);
	...
	for (int i = 0; i < num;i++)
	{
		MathLib::Vector3 closePt;
		ClosestPointWithTrans(pts[i], closePt);

		if (pts[i].distance(closePt) < dist)
		{
			states[i] = 1;
			isInteract = true;
		}
...
bool COBB::IsInsideWithTrans(const MathLib::Vector3 &p) const
{
...
bool COBB::PickByRayWithTrans(const MathLib::Vector3 &sp, const MathLib::Vector3 &dir, double &dPD) const
{
	MathLib::Vector3	e1, e2, p, s, q;
	double		t(0), u(0), v(0), w(0), tmp(0);
	int		pi = -1;
	...
	const std::vector<MathLib::Vector3> &VI = trans_vp;
	for (int i = 0; i < boxNumFace; i++) {
		const MathLib::Vector3 &v1 = trans_vp[boxFace[i][0]];
		const MathLib::Vector3 &v2 = trans_vp[boxFace[i][1]];
		const MathLib::Vector3 &v3 = trans_vp[boxFace[i][2]];
		e1.set(v2[0] - v1[0], v2[1] - v1[1], v2[2] - v1[2]);
		e2.set(v3[0] - v1[0], v3[1] - v1[1], v3[2] - v1[2]);
		p = dir.cross(e2);
		tmp = p.dot(e1);
...
MathLib::Vector3 COBB::GetTransformedCenter()
{
	MathLib::Vector3 trans_cent;
