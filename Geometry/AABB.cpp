...
void CAABB::Blend(const CAABB &obb) {
...
{
	if ( Abs(cent[0]-aabb.cent[0]) > (hsize[0]+aabb.hsize[0]) ) return false;
	if ( Abs(cent[1]-aabb.cent[1]) > (hsize[1]+aabb.hsize[1]) ) return false;
	if ( Abs(cent[2]-aabb.cent[2]) > (hsize[2]+aabb.hsize[2]) ) return false;
	return true;
}

bool CAABB::IsIntersect(const CAABB &aabb, double epsilon) const
{
	if ( (Abs(cent[0]-aabb.cent[0]) - (hsize[0]+aabb.hsize[0])) > epsilon ) return false;
	if ( (Abs(cent[1]-aabb.cent[1]) - (hsize[1]+aabb.hsize[1])) > epsilon ) return false;
	if ( (Abs(cent[2]-aabb.cent[2]) - (hsize[2]+aabb.hsize[2])) > epsilon ) return false;
	return true;
}
